...
// ====== Notificar estado a la app via BLE ======
void notificarEstadoBLE(const char* estado) {
  Serial.printf("[BLE] Notificando: %s\n", estado);

  // Apagar WiFi para usar BLE
  WiFi.disconnect(true);
  WiFi.mode(WIFI_OFF);
  delay(100);

  // Reiniciar BLE solo para notificar
  String nombreBLE = generarNombreBLE();
  BLEDevice::init(nombreBLE.c_str());
  ...
  pCharEstado->addDescriptor(new BLE2902());
  pCharEstado->setValue(estado);

  pService->start();
  BLEDevice::startAdvertising();

  // Esperar a que la app pueda reconectarse y leer el estado
  Serial.println("[BLE] Esperando que la app lea el estado...");
  delay(3000);
...
int loopModoBLE() {
  ...
  if (credencialesRecibidas) {
    ...
    if (probarConexionWiFi()) {
      ...
      mostrarPantallaBLE("OK! Avisando...");

      // Notificar a la app que se conecto exitosamente
      notificarEstadoBLE("CONECTADO");

      mostrarPantallaBLE("Reiniciando...");
      Serial.println("[BLE] Configuracion exitosa");
